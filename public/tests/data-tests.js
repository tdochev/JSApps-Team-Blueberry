describe('Data layer tests', () => {
    ...
    describe('Artists tests', () => {
        ...
        beforeEach(() => {
            jsonRequesterGetStub = sinon.stub(jsonRequester, 'get')
                .returns(Promise.resolve({ result: 'ARTISTS_WE_GET_IN_JSON' }));
        });
        afterEach(() => {
            jsonRequesterGetStub.restore();
        });

        it('expect getArtists function to return a Promise', () => {
            const promise = data.artists.get();
            expect(promise).to.be.an.instanceof(Promise);
        });

        it('expect getArtists function to make a GET request', (done) => {
            data.artists.get()
                .then(() => {
                    expect(jsonRequesterGetStub).to.have.been.calledOnce;
                })
        ...
        it('expect getArtists function to make a GET request to api/artists', (done) => {
            data.artists.get()
                .then(() => {
                    expect(jsonRequesterGetStub).to.have.been.calledWith('api/artists');
                })
                .then(done, done);
        });
        it('expect getArtists function to make a GET request to api/artists with authKey header', (done) => {
            const options = {
                headers: {
        ...
        it('expect getArtists function to return artists', (done) => {
            data.artists.get()
                .then(artists => {
                    expect(artists).to.equal('ARTISTS_WE_GET_IN_JSON');
                })
                .then(done, done);
        });
    });
    ...
    describe('Songs tests', () => {
        ...
        beforeEach(() => {
            jsonRequesterGetStub = sinon.stub(jsonRequester, 'get')
                .returns(Promise.resolve({ result: 'SONGS_WE_GET_IN_JSON' }));
        });
        afterEach(() => {
            jsonRequesterGetStub.restore();
        });

        it('expect getSongs function to return a Promise', () => {
            const promise = data.songs.get();
            expect(promise).to.be.an.instanceof(Promise);
        });

        it('expect getSongs function to make a GET request', (done) => {
            data.songs.get()
                .then(() => {
                    expect(jsonRequesterGetStub).to.have.been.calledOnce;
                })
        ...
        it('expect getSongs function to make a GET request to api/songs', (done) => {
            data.songs.get()
                .then(() => {
                    expect(jsonRequesterGetStub).to.have.been.calledWith('api/songs');
                })
                .then(done, done);
        });
        it('expect getSongs function to make a GET request to api/songs with authKey header', (done) => {
            const options = {
                headers: {
                    ['x-auth-key']: 'WHATEVER_AUTH_KEY_SEEMS_VALID'
                }
        ...
        it('expect getSongs function to return songs', (done) => {
            data.songs.get()
                .then(songs => {
                    expect(songs).to.equal('SONGS_WE_GET_IN_JSON');
                })
                .then(done, done);
        });
    });
